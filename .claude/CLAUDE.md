...
**Primary user:** Jose
**Active milestone:** M6 — Review & Memory (next)
...
| `.claude/agents/` | Agent instruction files | `dataset-analyser` ✅ (M2), `planner` ✅ (M3), `coder` ✅ (M4), `executor` ✅ (M5) |
...
| `src/` | Shared Python utilities | `analysis/` ✅ (M2), `planning/` ✅ (M3), `codegen/` ✅ (M4), `execution/` ✅ (M5) |
