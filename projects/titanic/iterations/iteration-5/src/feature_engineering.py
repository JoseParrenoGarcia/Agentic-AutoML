# %% [feature_engineering] Feature transforms for iteration 5
#
...
#
# Iteration 5 feature steps (from iteration-5.yaml):
#   1. drop_passengerid       — drop PassengerId (identifier, no signal)
#   2. drop_ticket            — drop Ticket (high cardinality alphanumeric)
#   3. drop_cabin             — drop raw Cabin after extracting HasCabin flag
#                               (deck-letter features dropped — importance < 0.002 in iter 4)
#   4. extract_title_from_name — extract Title from Name, map to 5 categories,
#                                one-hot encode (drop_first=True), then drop Name
...
#  10. create_has_cabin        — binary HasCabin flag (1 if Cabin non-null)
#  11. pclass_passthrough      — keep Pclass as-is
#
# Diff from iteration 4:
#   - Step 3 (drop_cabin) replaces extract_cabin_deck: removes deck-letter OHE
#     entirely (all deck features ranked 15-21, importance < 0.002 in iter 4).
#     Only the HasCabin binary flag is retained.
...
def fit_transform(
    ...
    # --- Step 4: Extract Title from Name, then drop Name ---
    ...
    # --- Step 3 & 10: Create HasCabin flag, then drop Cabin (no deck extraction) ---
    ...
    # --- Step 4 continued: one-hot encode Title (use training categories) ---
