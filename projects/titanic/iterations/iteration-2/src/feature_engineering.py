# %% [feature_engineering] Feature transforms for iteration 2
#
...
#
# Iteration 2 feature steps (from iteration-2.yaml):
#   1. drop_passengerid      — drop PassengerId (identifier, no signal)
#   2. drop_ticket           — drop Ticket (high cardinality alphanumeric)
#   3. extract_title_from_name — extract Title from Name, map to 5 categories, one-hot encode, drop Name
...
#  10. create_family_size    — FamilySize = SibSp + Parch + 1 (NEW in iter 2)
#  11. pclass_passthrough    — keep Pclass as-is
#  12. sibsp_parch_passthrough — keep SibSp and Parch as-is
#
# DIFF vs iteration 1:
#   - Step 3: NEW — extract Title from Name instead of dropping Name directly
#   - Step 10: NEW — create FamilySize feature
#   - Title categories fitted on train, stored in fitted_params
...
# ---------------------------------------------------------------------------
# Atomic transform helpers (carried forward from iteration 1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NEW in iteration 2: Title extraction helper
# ---------------------------------------------------------------------------

def _extract_title(name_series: pd.Series, title_map: dict | None = None) -> pd.Series:
    """
    Extract title from the Name column using regex, then map to consolidated
    categories: Mr, Mrs, Miss, Master, Rare.

    Args:
        name_series: The raw Name column.
        title_map:   Optional mapping from raw title to consolidated title.
                     If None, uses the default mapping.

    Returns:
        Series of consolidated title strings.
    """
    ...
    # Extract text before the first period: "Braund, Mr. Owen" -> "Mr"
    raw_titles = name_series.str.extract(r",\s*([^.]+)\.", expand=False).str.strip()
    consolidated = raw_titles.map(title_map).fillna("Rare")
    return consolidated
...
def fit_transform(
    ...
    # --- Step 3: Extract Title from Name, then drop Name (NEW in iter 2) ---
    ...
    # --- Step 1 & 2: Drop identifier/high-cardinality columns ---
    ...
    # --- Step 10: create_family_size (NEW in iter 2) ---
    df["FamilySize"] = df["SibSp"] + df["Parch"] + 1

    # --- Step 3 continued: one-hot encode Title (drop first) ---
    title_dummies = _onehot_encode(df["Title"], fitted_params["title_categories"], drop_first=True)
    df = df.drop(columns=["Title"])
    df = pd.concat([df, title_dummies], axis=1)

    # --- Steps 11 & 12: Pclass, SibSp, Parch pass through unchanged ---
    # No action needed.
