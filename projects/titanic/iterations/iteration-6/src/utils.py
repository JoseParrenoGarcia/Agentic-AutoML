...
def set_seed(seed: int) -> None:
    """Set random seed for reproducibility across Python random, numpy, and sklearn."""
    random.seed(seed)
    try:
        import numpy as np
        np.random.seed(seed)
...
def capture_warnings(stage_name: str, metadata: "PipelineMetadata") -> Generator:
    """
    Context manager that captures Python warnings during a pipeline stage and
    records them in PipelineMetadata.
